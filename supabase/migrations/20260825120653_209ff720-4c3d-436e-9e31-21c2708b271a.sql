-- allow internal system operations (guest checkout) to pass inventory permission checks
...
-- pick the best location for a reservation inside a shop
...
CREATE OR REPLACE FUNCTION public.cart_start_checkout(
  _org uuid, _shop uuid, _cart uuid, _snapshot uuid, _actor uuid,
  _email text DEFAULT NULL, _ttl_minutes integer DEFAULT 20, _idem text DEFAULT NULL
...
CREATE OR REPLACE FUNCTION public.cart_cancel_checkout(
  _org uuid, _session uuid, _actor uuid, _status public.checkout_session_status DEFAULT 'cancelled',
...
CREATE OR REPLACE FUNCTION public.cart_expire_checkout_sessions(_org uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
...
REVOKE EXECUTE ON FUNCTION public.cart_start_checkout(uuid,uuid,uuid,uuid,uuid,text,integer,text) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.cart_cancel_checkout(uuid,uuid,uuid,public.checkout_session_status,text) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.cart_expire_checkout_sessions(uuid) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.cart_release_session_reservations(uuid,uuid) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.cart_pick_location(uuid,uuid,uuid,integer) FROM anon, authenticated;