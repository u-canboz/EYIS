...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (SELECT checkout_session_id, min(shop_id) AS shop_id, count(*) AS n FROM public.orders
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    WHERE r.organization_id = _org_id AND r.status = 'active'
      AND NOT EXISTS (SELECT 1 FROM public.checkout_sessions cs
        WHERE cs.id = r.reference_id AND cs.status IN ('open','validated','awaiting_payment'))
    LIMIT 50) t;
  findings := findings || f;
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (SELECT reference_id, min(shop_id) AS shop_id, count(*) AS n FROM public.inventory_movements
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (
      SELECT m.reference_id, min(m.shop_id) AS shop_id,
    ...
    JOIN public.inventory_transfers t ON t.id = g.reference_id AND t.status = 'completed'
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (SELECT tax_snapshot_id, min(shop_id) AS shop_id, count(*) AS n FROM public.orders
  ...
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'public' AND c.relname = 'tax_snapshots'
      AND p.proname = 'tax_snapshot_immutable' AND NOT t.tgisdisabled
  ) THEN
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (
      SELECT fi.order_item_id, min(f.shop_id) AS shop_id, sum(fi.quantity) AS n
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (SELECT shipment_id, min(shop_id) AS shop_id, count(*) AS n FROM public.shipping_labels
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (
      SELECT ri.order_item_id, min(r.shop_id) AS shop_id, sum(ri.quantity_approved) AS n
  ...
  SELECT coalesce(jsonb_agg(x), '[]'::jsonb) INTO f FROM (
    ...
    FROM (
      SELECT source_event_id, communication_rule_id, recipient_address, min(shop_id) AS shop_id, count(*) AS n
