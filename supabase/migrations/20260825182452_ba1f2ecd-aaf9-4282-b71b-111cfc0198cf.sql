REVOKE EXECUTE ON FUNCTION public.comm_template_version_guard() FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.communication_snapshot_guard() FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.communication_provider_event_guard() FROM anon, authenticated;
...
END; $$;
REVOKE EXECUTE ON FUNCTION public.comm_ensure_shop_defaults(uuid, uuid) FROM anon, authenticated;

DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT id, organization_id FROM public.shops LOOP
    PERFORM public.comm_ensure_shop_defaults(r.organization_id, r.id);
  END LOOP;
END $$;