...
export async function listProviderConfigs(organizationId: string, shopId?: string | null): Promise<ProviderConfigView[]> {
  const admin = await getAdmin();
  let query = admin.from("shipping_provider_configs").select("*").eq("organization_id", organizationId).order("priority");
  ...
  return ((data ?? []) as Row[]).map((r) => ({
    ...
    hasWebhookSecret: Boolean((r['configuration_reference'] as Row | null)?.['webhook_secret_name']),
  }));
}
...
export async function saveProviderConfig(input: {
  ...
  const payload = {
    ...
    configuration_reference: input.webhookSecretName ? { webhook_secret_name: input.webhookSecretName } : {},
  };
...
export async function listPackagePresets(organizationId: string, shopId?: string | null): Promise<PackagePresetView[]> {
  const admin = await getAdmin();
  const { data, error } = await admin
...
export async function deletePackagePreset(organizationId: string, id: string) {
  const admin = await getAdmin();
  const { error } = await admin.from("package_presets").delete().eq("id", id).eq("organization_id", organizationId);
  if (error) throw new Error(error.message);
  return { ok: true };
}
...
async function loadShipContext(organizationId: string, fulfillmentId: string, packageId: string) {
  ...
  const [{ data: pkg }, { data: order }, { data: addressRow }] = await Promise.all([
    admin.from("packages").select("*").eq("id", packageId).eq("fulfillment_id", fulfillmentId).maybeSingle(),
    admin.from("orders").select("order_number, email").eq("id", f['order_id'] as string).maybeSingle(),
    admin
  ...
  const address: CarrierAddress = {
    name: [raw['firstName'], raw['lastName']].filter(Boolean).join(" ").trim() || (raw['name'] ?? ""),
  ...
  if (!address.name || !address.line1 || !address.postalCode || !address.city || !address.countryCode) {
    throw new CarrierError("invalid_address", "Die Lieferadresse der Bestellung ist unvollständig.", false);
  }
...
async function loadActiveConfig(organizationId: string, shopId: string, provider: string) {
  ...
  const { data } = await admin
    ...
    .eq("status", "active")
    .maybeSingle();
  if (!data) throw new CarrierError("not_supported", `Versanddienstleister ${provider} ist nicht aktiv.`, false);
  return data as Row;
}
...
async function recordError(shipmentId: string, code: string, message: string) {
  ...
  await admin
    .from("shipments")
    .update({ last_error: { code, message, at: new Date().toISOString() } as never, status: "exception" })
    .eq("id", shipmentId);
}
...
export async function createShipmentWithLabel(input: {
  ...
  const { data: created, error: createError } = await admin.rpc("ship_create" as never, {
  ...
  if (!existingLabel) {
    ...
    try {
      ...
      const label = await carrier.createLabel({ ...carrierInput, providerShipmentId: shipment.providerShipmentId });
      ...
      const { error: recordError2 } = await admin.rpc("ship_record_label" as never, {
        _org: input.organizationId,
      ...
      const message = e instanceof Error ? e.message : "Unbekannter Fehler beim Versanddienstleister.";
...
export async function loadShipment(organizationId: string, shipmentId: string): Promise<ShipmentView> {
  const admin = await getAdmin();
  const { data, error } = await admin
    .from("shipments")
    .select("*")
...
export async function markShipped(input: {
  ...
  const admin = await getAdmin();
  const { data, error } = await admin.rpc("ship_mark_shipped" as never, {
    _org: input.organizationId,
  ...
  if (shipment.providerShipmentId) {
    ...
    if (carrier.capabilities.supportsCancellation && carrier.cancelShipment) {
      try {
        ...
        const message = e instanceof Error ? e.message : "Stornierung beim Dienstleister fehlgeschlagen.";
        throw new Error(message);
      }
    }
  }
  const { data, error } = await admin.rpc("ship_cancel" as never, {
    _org: input.organizationId,
    _shipment: input.shipmentId,
    _actor: input.actorId,
...
export async function refreshTracking(organizationId: string, shipmentId: string) {
  ...
  const result = await recordTrackingEvents(organizationId, shipmentId, shipment.carrierProvider, snapshot.events);
