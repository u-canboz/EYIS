...
export async function listFulfillmentQueue(input: {
  ...
  const [items, packages, shipments, orders, locations] = await Promise.all([
    admin
      ...
      .select("fulfillment_id, quantity, picked_quantity, packed_quantity, shipped_quantity")
  ...
  const locationById = new Map(
    ((locations.data ?? []) as Row[]).map((l) => [l["id"] as string, l["name"] as string]),
  );
  ...
  const list = rows.map((f) => {
    ...
    return {
      ...
      locationName: f["location_id"]
        ? (locationById.get(f["location_id"] as string) ?? null)
        : null,
  ...
  const filtered = list.filter((f) => {
    ...
    return (
      ...
      (f.trackingNumber ?? "").toLowerCase().includes(term)
    );
  });
  return filtered;
}
...
export async function loadFulfillment(
  ...
  const { data: orderItems } = orderItemIds.length
    ? await admin
        .from("order_items")
        .select("id, title_snapshot, variant_title_snapshot, sku_snapshot")
        .in("id", orderItemIds)
    : { data: [] as Row[] };
  ...
  const labelByShipment = new Map(
    ((labelRows.data ?? []) as Row[]).map((l) => [
      l["shipment_id"] as string,
      l["storage_path"] as string,
    ]),
  );
  ...
  const packages: PackageView[] = ((packageRows.data ?? []) as Row[]).map((p) => {
    ...
    return {
      ...
      shipment: shipment
        ? mapShipment(shipment, labelByShipment.get(shipment["id"] as string) ?? null)
        : null,
    };
  });
...
export async function suggestAllocation(
  ...
  const invItemByVariant = new Map(
    ((invItems ?? []) as Row[]).map((r) => [r["variant_id"] as string, r["id"] as string]),
  );
  ...
  const lines: AllocationLine[] = items.map((i) => {
    ...
    const options = locations.map((l) => {
      ...
      return {
        ...
        available: level
          ? num(level["on_hand"]) - num(level["reserved"]) - num(level["damaged"])
          : 0,
    ...
    const sorted = [...options].sort(
      (a, b) =>
        Number(b.available >= open) - Number(a.available >= open) ||
...
export function nextAction(view: FulfillmentView): NextAction {
  if (view.status === "cancelled")
    return { action: "done", label: "Storniert", fulfillmentId: view.id };
  if (view.status === "delivered")
    return { action: "done", label: "Abgeschlossen", fulfillmentId: view.id };
  ...
  if (view.status === "packed") {
    ...
    if (withoutShipment.length)
      return { action: "create_label", label: "Label erstellen", fulfillmentId: view.id };
