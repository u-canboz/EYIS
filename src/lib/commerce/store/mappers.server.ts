  ...
  return {
    ...
    tax: {
      ...
      breakdown: view.tax.breakdown.map((b) => ({
        rate: Number((b as unknown as { rate?: number }).rate ?? 0),
        label: String((b as unknown as { label?: string }).label ?? ""),
        netMinor: Number((b as unknown as { netMinor?: number }).netMinor ?? 0),
        taxMinor: Number((b as unknown as { taxMinor?: number }).taxMinor ?? 0),
      })),
