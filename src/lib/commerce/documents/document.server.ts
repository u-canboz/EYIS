...
export async function listInvoices(input: {
  ...
  const orderMap = new Map(
    ((orders.data ?? []) as Row[]).map((o) => [o["id"] as string, o["order_number"] as string]),
  );
  const pdfSet = new Set(
    ((files.data ?? []) as Row[])
      .filter((f) => f["format"] === "pdf")
      .map((f) => f["document_id"] as string),
  );
...
export async function loadOrderDocuments(organizationId: string, orderId: string) {
  ...
  const [invoices, deliveryNotes] = await Promise.all([
    admin
      .from("invoices")
      .select(
        "id, invoice_number, status, total_gross_minor, credited_minor, currency_code, issue_date, created_at",
      )
      .eq("organization_id", organizationId)
      .eq("order_id", orderId)
      .order("created_at", { ascending: false }),
  ...
  const files = invoiceRows.length
    ? ((
        await admin
          ...
          .in(
            "document_id",
            invoiceRows.map((i) => i["id"] as string),
          )
      ).data ?? [])
    : [];
  const pdfSet = new Set(
...
export async function listDeliveryNotes(
  ...
  const orders = orderIds.length
    ...
    : [];
  const orderMap = new Map(
    ((orders as Row[]) ?? []).map((o) => [o["id"] as string, o["order_number"] as string]),
  );
...
export async function loadSetup(organizationId: string, shopId: string): Promise<DocumentSetup> {
  ...
  if (
    ...
    !String(s["postal_code"] ?? "").trim() ||
    !String(s["city"] ?? "").trim()
  )
    missing.push("address");
  if (!s || (!String(s["tax_number"] ?? "").trim() && !String(s["vat_id"] ?? "").trim()))
    missing.push("tax");
  if (!((sequences.data ?? []) as Row[]).some((q) => q["document_type"] === "invoice"))
  ...
  return {
    settings: s
      ? {
          ...
          invoiceCreationStrategy: s[
            "invoice_creation_strategy"
          ] as InvoiceSettings["invoiceCreationStrategy"],
    ...
    branding: b
      ? {
          ...
          bankDetails: (b["bank_details"] ?? {}) as Record<
            string,
...
export async function issueInvoice(input: {
  ...
  const result = await rpc<{ invoice_id: string; invoice_number: string; issued: boolean }>(
    "invoice_issue",
    {
      _org: input.organizationId,
...
export async function createDeliveryNote(input: {
  ...
  const result = await rpc<{ delivery_note_id: string; document_number: string }>(
...
async function storeFile(input: {
  ...
  const { data: existing } = await admin
    ...
    .order("version", { ascending: false })
    .limit(1);
  const version = ((existing ?? []) as Row[]).length
    ? num(((existing ?? []) as Row[])[0]!["version"]) + 1
    : 1;
...
export async function generateInvoicePdf(input: {
  ...
  if (!input.force && invoice.files.some((f) => f.format === "pdf")) {
    return {
      path: invoice.files.find((f) => f.format === "pdf")!.storagePath!,
      version: invoice.files[0]!.version,
    };
  }
...
export async function generateDeliveryNotePdf(input: {
  ...
  const row = data as Row;
  const orderNumber = ((row["metadata"] ?? {}) as Record<string, unknown>)["order_number"] as
    string | undefined;
