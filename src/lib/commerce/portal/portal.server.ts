...
export type PortalTracking = {
  ...
  events: { code: string; description: string | null; occurredAt: string; location: string | null }[];
};
...
export async function loadPortalOrder(orderId: string): Promise<PortalOrderDetail> {
  ...
  const [items, addresses, invoices, creditNotes, deliveryNotes, fulfillments, returns] = await Promise.all([
    admin.from("order_items").select("*").eq("order_id", orderId).order("created_at"),
    admin.from("order_addresses").select("*").eq("order_id", orderId),
    admin.from("invoices").select("id, invoice_number, issued_at, total_gross_minor, status").eq("order_id", orderId),
    admin.from("credit_notes").select("id, credit_note_number, issued_at, total_gross_minor, status").eq("order_id", orderId),
    admin.from("delivery_notes").select("id, document_number, issued_at, status").eq("order_id", orderId),
    admin.from("fulfillments").select("id").eq("order_id", orderId),
    admin.from("returns").select("id, return_number, status, requested_at").eq("order_id", orderId).order("requested_at", { ascending: false }),
  ]);

  const invoiceRows = (invoices.data ?? []) as Row[];
  const fulfillmentIds = new Set(((fulfillments.data ?? []) as Row[]).map((f) => f['id'] as string));
  ...
  if (fulfillmentIds.size) {
    ...
    const { data: events } = shipmentRows.length
      ? await admin
          ...
          .in("shipment_id", shipmentRows.map((s) => s['id'] as string))
...
export async function signPortalDocument(kind: PortalDocument["kind"], documentId: string) {
  const admin = await getAdmin();
  const table = kind === "invoice" ? "invoices" : kind === "credit_note" ? "credit_notes" : "delivery_notes";
  const { data } = await admin
    .from("document_files")
