...
export function FilterBar({ search, filters, activeCount = 0, onReset, className }: Props) {
  ...
  return (
    <div className={cn("flex min-w-0 flex-col gap-3", className)}>
      ...
      <div className="hidden min-w-0 flex-wrap items-center gap-2 rounded-xl border border-border bg-card p-2  md:flex [&_button[role=combobox]]:h-11 [&_input]:h-11 [&_button[data-slot=select-trigger]]:h-11">
        {search ? <div className="min-w-56 flex-1">{search}</div> : null}
        {filters}
        {onReset && activeCount > 0 ? (
          <Button variant="ghost" className="h-11 shrink-0" onClick={onReset}>
            Zurücksetzen
          </Button>
        ) : null}
      </div>
