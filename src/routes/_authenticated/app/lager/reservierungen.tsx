  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Reservierter Bestand ist physisch vorhanden, aber nicht mehr verkäuflich."
      />
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger aria-label="Status filtern" className="w-48">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      {reservationsQuery.isLoading ? (
        ...
        <ErrorState
          description="Reservierungen konnten nicht geladen werden."
          action={
            <Button variant="outline" onClick={() => void reservationsQuery.refetch()}>
              Erneut laden
            </Button>
          }
        />
        ...
        <EmptyState
          title="Keine Reservierungen in diesem Status"
          description="Wähle einen anderen Status, um frühere Reservierungen zu sehen."
        />
      ) : (
        <>
          <RecordCardList>
            {rows.map((row) => (
              <RecordCard
                key={row.id}
                title={row.inventory_items?.product_variants?.products?.name ?? "Reservierung"}
                subtitle={row.inventory_items?.product_variants?.title ?? undefined}
                badges={
                ...
                fields={[
                  { label: "Menge", value: row.quantity },
                  { label: "Nachbestellt", value: row.backordered_quantity },
                  {
                    label: "Referenz",
                    value: `${row.reference_type === "checkout_session" ? "Checkout" : (row.reference_type ?? "—")}${row.reference_id ? ` · ${row.reference_id.slice(0, 8)}` : ""}`,
                  },
                  {
                    label: "Läuft ab",
                    value: row.expires_at
                      ? new Date(row.expires_at).toLocaleString("de-DE")
                      : "Unbefristet",
                  },
                ]}
                actions={
